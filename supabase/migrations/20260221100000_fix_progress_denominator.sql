-- ============================================================================
-- HOTFIX: Progress denominator must include ALL tasks/milestones
-- ============================================================================
-- BUG: The three progress RPCs (get_project_progress_asof,
--      get_projects_progress_asof, get_project_progress_hierarchy)
--      derived weight denominators only from tasks/milestones reachable
--      via the subtasks table. Tasks or milestones with no deliverables
--      were excluded from the denominator, inflating the progress of
--      siblings that DID have deliverables.
--
-- FIX: Remove the EXISTS(subtasks) filter from mw_sum and tw_sums.
--      Query milestones/tasks tables directly so entities without
--      deliverables still consume weight in the denominator (contributing
--      0 progress but preventing inflation).
--
-- NOTE: get_project_scurve and get_project_progress_today already use
--       the correct pattern (no EXISTS filter). Only these 3 RPCs needed
--       fixing.
-- ============================================================================


-- --------------------------------------------------------------------------
-- 1. get_project_progress_asof  (fix mw_sum and tw_sums)
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- FIX: include ALL milestones, not just those with deliverables
  mw_sum AS (
    SELECT COALESCE(SUM(ml.weight), 0) AS total
    FROM milestones ml
    WHERE ml.project_id = p_project_id
  ),
  -- FIX: include ALL tasks, not just those with deliverables
  tw_sums AS (
...
-- --------------------------------------------------------------------------
-- 2. get_projects_progress_asof  (fix mw_sum and tw_sums)
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 3. get_project_progress_hierarchy  (fix tw_sums and mw_sum)
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  -- FIX: include ALL tasks, not just those reachable via raw_deliv
  tw_sums AS (
    SELECT tk.milestone_id, COALESCE(SUM(tk.weight), 0) AS total
    FROM tasks tk
    JOIN milestones ml ON ml.id = tk.milestone_id
  ...
  -- FIX: include ALL milestones, not just those reachable via raw_deliv
  mw_sum AS (
    SELECT COALESCE(SUM(ml.weight), 0) AS total
    FROM milestones ml
    WHERE ml.project_id = p_project_id
  ...
  -- FIX: get task weights from the tasks table directly
  task_weights AS (
    SELECT tk.id AS task_id, tk.weight AS tw
    FROM tasks tk
    JOIN milestones ml ON ml.id = tk.milestone_id
    WHERE ml.project_id = p_project_id
  ),
  -- FIX: get milestone weights from the milestones table directly
  milestone_weights AS (
    SELECT ml.id AS milestone_id, ml.weight AS mw
    FROM milestones ml
    WHERE ml.project_id = p_project_id
  ),
