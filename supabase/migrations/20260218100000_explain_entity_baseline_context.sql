-- Migration: expose planned/actual in explain_entity response
--
-- The RPC already computes v_planned and v_actual internally but doesn't
-- return them. Adding them to the final JSON enables the frontend to build
-- contextual summaries ("ahead of schedule" vs "behind schedule") instead
-- of relying solely on worst-case risk_state.
--
-- This is a CREATE OR REPLACE — no schema changes, just two extra fields
...
BEGIN
  ...
  --
  -- Hard delays (always DELAYED): overdue completion, critical-path late
  -- Context-aware: BASELINE_SLIP is DELAYED only when behind schedule,
  --   otherwise AT_RISK (a slip while ahead is a risk, not a delay)
  IF v_reasons @> '[{"code":"PLANNED_COMPLETE_BUT_NOT_DONE"}]'::jsonb
     OR v_reasons @> '[{"code":"CRITICAL_TASK_LATE"}]'::jsonb THEN
    v_reason_status := 'DELAYED';
  ELSIF v_reasons @> '[{"code":"BASELINE_SLIP"}]'::jsonb THEN
    -- Baseline slip + behind schedule = delayed; ahead = at risk
  ...
  --    Now includes planned/actual (0-1 scale) so the frontend can
  --    build contextual summaries without re-fetching progress.
  -- ---------------------------------------------------------------
  RETURN jsonb_build_object(
    ...
    'asof',        p_asof,
    'status',      v_status,
    'planned',     round(v_planned, 4),
    'actual',      round(v_actual, 4),
    'reasons',     v_reasons,
...
COMMENT ON FUNCTION public.explain_entity(text, bigint, date) IS
  'Phase 4.1 v3 — Deterministic explainability RPC. Now exposes planned/actual '
  'progress (0-1 scale) in the response for contextual frontend summaries. '
